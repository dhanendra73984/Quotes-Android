...
import com.example.awesomequotesapp.Adapters.MyFavQuotesRecyclerViewAdapter;
...
public class MyFavQuotesFragment extends Fragment {

    RecyclerView recyclerView;
    List<Quote> quoteList;
    MyFavQuotesRecyclerViewAdapter adapter;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        return inflater.inflate(R.layout.fragment_my_fav_quotes, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new MyFavQuotesRecyclerViewAdapter(getContext(),quoteList);
    ...
    private void getFavQuotes()
    {

        int id = getContext().getSharedPreferences("quotes_app", Context.MODE_PRIVATE).getInt("uid",0);
        quoteList.clear();
        RetroClient.getInstance().getApi().getFavQuotes(id).enqueue(new Callback<JsonObject>() {
            @Override
